...
namespace ZiCoffe.DAO
{
    public class PositionDAO
    {
        ...
        {
            List<EmployeesDTO> employeesList = EmployeesDAO.Instance.GetEmployeesList(maChucVu);
            foreach (EmployeesDTO item in employeesList)
            {
                EmployeesDAO.Instance.DeleteEmployees(maChucVu);
                LoginDAO.Instance.DeleteAccount(item.TenDangNhap);
            }
